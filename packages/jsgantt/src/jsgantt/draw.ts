...
export const GanttChart = function (pDiv, pFormat) {
  ...
  this.vMinGpLen = 8;
  this.vScrollTo = '';
  this.vHourColWidth = 32;
  this.vDayColWidth = 32;
  this.vWeekColWidth = 100;
  this.vMonthColWidth = 200;
  this.vQuarterColWidth = 300;
  this.vRowHeight = 32;
  this.vTodayPx = -1;
  this.vLangs = lang;
  ...
  this.drawListBody = function (vLeftHeader) {
    ...
    let vTmpContentTabWrapper = newNode(
      vTmpContentTabOuterWrapper,
      'div',
      null,
      'gtasktablewrapper',
    );
    ...
    for (let i = 0; i < this.vTaskList.length; i++) {
      ...
      if (
        !(this.vTaskList[i].getParItem() && this.vTaskList[i].getParItem().getGroup() == 2) ||
        ...
        if (this.vTaskList[i].getGroup() == 1) {
          ...
          let vTmpSpan = newNode(
            vTmpDiv,
            'span',
            this.vDivId + 'group_' + vID,
            'gfoldercollapse',
            this.vTaskList[i].getOpen() == 1 ? '-' : '+',
          );
    ...
    if (this.vTaskList.length == 0) {
      let totalColumns = this.getColumnOrder().filter(
        (column) => this[column] == 1 || column === 'vAdditionalHeaders',
      ).length;
    ...
    while (vTmpDate.getTime() <= vMaxDate.getTime()) {
      ...
      if (this.vFormat == 'day') {
        ...
        if (this.vShowEndWeekDate == 1)
          vCellContents +=
            ' - ' +
            formatDateStr(vTmpDate, this.vDayMajorDateDisplayFormat, this.vLangs[this.vLang]);
        ...
        newNode(
          ...
          null,
          null,
          formatDateStr(vTmpDate, this.vWeekMajorDateDisplayFormat, this.vLangs[this.vLang]),
          vColWidth,
        );
        ...
        newNode(
          ...
          null,
          null,
          formatDateStr(vTmpDate, this.vMonthMajorDateDisplayFormat, this.vLangs[this.vLang]),
          vColWidth * vColSpan,
        );
        ...
        newNode(
          ...
          null,
          null,
          formatDateStr(vTmpDate, this.vQuarterMajorDateDisplayFormat, this.vLangs[this.vLang]),
          vColWidth * vColSpan,
        );
        vTmpDate.setFullYear(vTmpDate.getFullYear() + 1, 0, 1);
        ...
        if (
          vTmpDate.getFullYear() == vMaxDate.getFullYear() &&
        ...
        newNode(
          ...
          null,
          null,
          formatDateStr(vTmpDate, this.vHourMajorDateDisplayFormat, this.vLangs[this.vLang]),
          vColWidth * vColSpan,
        );
        vTmpDate.setHours(0);
        vTmpDate.setDate(vTmpDate.getDate() + 1);
      ...
      if (this.vFormat == 'day') {
        ...
        if (vTmpDate <= vMaxDate) {
          ...
          newNode(
            ...
            null,
            null,
            formatDateStr(vTmpDate, this.vDayMinorDateDisplayFormat, this.vLangs[this.vLang]),
            vColWidth,
          );
          vNumCols++;
        }
        ...
        if (vTmpDate <= vMaxDate) {
          ...
          newNode(
            ...
            null,
            null,
            formatDateStr(vTmpDate, this.vWeekMinorDateDisplayFormat, this.vLangs[this.vLang]),
            vColWidth,
          );
          vNumCols++;
        }
        ...
        if (vTmpDate <= vMaxDate) {
          ...
          newNode(
            ...
            null,
            null,
            formatDateStr(vTmpDate, this.vMonthMinorDateDisplayFormat, this.vLangs[this.vLang]),
            vColWidth,
          );
          vNumCols++;
        }
        ...
        if (vTmpDate <= vMaxDate) {
          ...
          newNode(
            ...
            null,
            null,
            formatDateStr(vTmpDate, this.vQuarterMinorDateDisplayFormat, this.vLangs[this.vLang]),
            vColWidth,
          );
          vNumCols++;
        }
        ...
        for (let i = vTmpDate.getHours(); i < 24; i++) {
          ...
          if (vTmpDate <= vMaxDate) {
            ...
            newNode(
              ...
              null,
              null,
              formatDateStr(vTmpDate, this.vHourMinorDateDisplayFormat, this.vLangs[this.vLang]),
              vColWidth,
            );
            vNumCols++;
          }
  ...
  this.drawCharBody = function (
    ...
    const vTmpDiv = newNode(
      vRightTable,
      'div',
      this.vDivId + 'gchartbody',
      'gchartgrid gcontainercol',
    );
    this.setChartBody(vTmpDiv);
    const vTmpTab = newNode(
      vTmpDiv,
      'table',
      this.vDivId + 'chartTable',
      'gcharttable',
      null,
      vTaskLeftPx,
    );
    ...
    for (i = 0; i < this.vTaskList.length; i++) {
      let curTaskStart = this.vTaskList[i].getStart()
        ? this.vTaskList[i].getStart()
        : this.vTaskList[i].getPlanStart();
      let curTaskEnd = this.vTaskList[i].getEnd()
        ? this.vTaskList[i].getEnd()
        : this.vTaskList[i].getPlanEnd();

      const vTaskLeftPx = getOffset(
        vMinDate,
        curTaskStart,
        vColWidth,
        this.vFormat,
      ...
      if (this.vTaskList[i].getMile() && !vComb) {
        ...
        if (this.vTaskList[i].getGroup()) {
          ...
          const vTmpRow = newNode(
            ...
            'tr',
            this.vDivId + 'childrow_' + vID,
            (this.vTaskList[i].getGroup() == 2 ? 'glineitem gitem' : 'ggroupitem ggroup') +
              this.vFormat,
            null,
            null,
            null,
          ...
          if (this.vTaskList[i].getGroup() == 1) {
            ...
            if (vTaskWidth >= this.vMinGpLen * 2)
              newNode(vTmpDiv, 'div', null, this.vTaskList[i].getClass() + 'endpointright');
          ...
          if (this.vTaskList[i].getStartVar()) {
            ...
            if (this.vTaskList[i].getBarText()) {
              newNode(
                vTmpDiv2,
                'span',
                this.vDivId + 'tasktextbar_' + vID,
                'textbar',
                this.vTaskList[i].getBarText(),
                this.vTaskList[i].getCompRestStr(),
              );
            }
            this.vTaskList[i].setTaskDiv(vTmpDiv2);
          }
          ...
          if (
            ...
            const vTmpPlanDiv2 = newNode(
              ...
              'div',
              this.vDivId + 'taskbar_' + vID,
              this.vTaskList[i].getClass() + ' gplan',
              null,
              vTaskPlanRightPx,
            );
          ...
          if (
            !vComb ||
            (vComb && this.vTaskList[i].getParItem().getEnd() == this.vTaskList[i].getEnd())
          )
            vCaptClass = 'gcaption';
      ...
      if (this.vTaskList[i].getTaskDiv() && vTmpDiv) {
        ...
        const { component, callback } = this.createTaskInfo(
          this.vTaskList[i],
          this.vTooltipTemplate,
        );
    ...
    if (vSingleCell) {
      ...
      const vTmpTFootTCellTable = newNode(
        vTmpTFootTCell,
        'table',
        null,
        'gcharttableh',
  ...
  this.drawColsChart = function (
    ...
    if (
      ...
      columnCurrentDay =
        Math.floor(calculateCurrentDateOffset(curTaskStart, curTaskEnd) / onePeriod) - 1;
    }
  ...
  this.Draw = function () {
    ...
    vMinDate = getMinDate(
      this.vTaskList,
      this.vFormat,
      this.getMinDate() && coerceDate(this.getMinDate()),
    );
    vMaxDate = getMaxDate(
      this.vTaskList,
      this.vFormat,
      this.getMaxDate() && coerceDate(this.getMaxDate()),
    );
     ...
     * CHART GRID
     */
    const { vRightTable } = this.drawCharBody(
      vTaskLeftPx,
      vTmpContentTabWrapper,
      gChartLbl,
      gListLbl,
    ...
    if (
      this.vEvents.onLineContainerHover &&
      typeof this.vEvents.onLineContainerHover === 'function'
    ) {
    ...
    if (this.vScrollTo != '') {
      ...
      if (this.vScrollTo.substr && this.vScrollTo.substr(0, 2) == 'px') {
        ...
        vScrollPx =
          getOffset(vMinDate, vScrollDate, vColWidth, this.vFormat, this.vShowWeekends) - 30;
      }
