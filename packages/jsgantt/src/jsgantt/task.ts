  ...
  if (pStart != null && pStart != '') {
    vStart =
      pStart instanceof Date
        ? pStart
        : parseDateStr(document.createTextNode(pStart).data, vGantt.getDateInputFormat());
  ...
  if (pEnd != null && pEnd != '') {
    vEnd =
      pEnd instanceof Date
        ? pEnd
        : parseDateStr(document.createTextNode(pEnd).data, vGantt.getDateInputFormat());
    vGroupMinEnd = vEnd;
  }
  ...
  if (pPlanStart != null && pPlanStart != '') {
    vPlanStart =
      pPlanStart instanceof Date
        ? pPlanStart
        : parseDateStr(document.createTextNode(pPlanStart).data, vGantt.getDateInputFormat());
  ...
  if (pPlanEnd != null && pPlanEnd != '') {
    vPlanEnd =
      pPlanEnd instanceof Date
        ? pPlanEnd
        : parseDateStr(document.createTextNode(pPlanEnd).data, vGantt.getDateInputFormat());
    ...
    if (template) {
      ...
      if (this.vShowTaskInfoStartDate == 1) {
        ...
        newNode(
          ...
          null,
          'gTaskText',
          formatDateStr(pTask.getStart(), this.vDateTaskDisplayFormat, this.vLangs[this.vLang]),
        );
      }
      if (this.vShowTaskInfoEndDate == 1) {
        ...
        newNode(
          ...
          null,
          'gTaskText',
          formatDateStr(pTask.getEnd(), this.vDateTaskDisplayFormat, this.vLangs[this.vLang]),
        );
      }
      if (this.vShowTaskInfoDur == 1 && !pTask.getMile()) {
        ...
        newNode(
          ...
          null,
          'gTaskText',
          pTask.getDuration(this.vFormat, this.vLangs[this.vLang]),
        );
      }
      ...
      //额外信息
      const { toolTipsInfo } = pTask.getDataObject();
      if (Array.isArray(toolTipsInfo)) {
        toolTipsInfo.forEach((element) => {
          vTmpDiv = newNode(vTaskInfo, 'div', null, 'gTILine gTIr');
          newNode(vTmpDiv, 'span', null, 'gTaskLabel', element.key + ': ');
          newNode(vTmpDiv, 'span', null, 'gTaskText', element.value);
        });
      }
    }
  };
  ...
  if (typeof templateStrOrFn === 'function') {
    callback = () => {
      ...
      if (!strOrPromise || typeof strOrPromise === 'string') {
        ...
        setupTemplate(
          this.vLangs[this.vLang]['tooltipLoading'] || this.vLangs['en']['tooltipLoading'],
        );
...
export const processRows = function (pList, pID, pRow, pLevel, pOpen, pUseSort, vDebug = false) {
  ...
  if (pRow >= 0) {
    ...
    if (
      pList[pRow].getGroupMinPlanEnd() != null &&
