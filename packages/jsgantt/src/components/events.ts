...
export const folder = function (pID, ganttObj) {
  ...
  ganttObj.DrawDependencies(vDebug);
  if (vDebug) {
    const ad = new Date();
...
export var addListener = function (eventName, handler, control) {
  ...
  if (pGanttChartObj.getUseToolTip()) {
    if (pGanttChartObj.vTool == null) {
      ...
      pGanttChartObj.vTool.style.left =
        Math.floor((e ? e.clientX : (<MouseEvent>window.event).clientX) / 2) + 'px';
      pGanttChartObj.vTool.style.top =
        Math.floor((e ? e.clientY : (<MouseEvent>window.event).clientY) / 2) + 'px';
      addListener(
        'mouseover',
        function () {
    ...
    function (e) {
      if (isShowingTooltip || !callback) {
        ...
        if (promise && promise.then) {
          promise.then(() => {
            if (
              pGanttChart.vTool.vToolCont.getAttribute('showing') === pObj2.id &&
