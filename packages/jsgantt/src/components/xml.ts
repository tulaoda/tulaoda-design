...
export const parseXML = function (pFile, pGanttVar) {
  return makeRequest(pFile, false, false).then((xmlDoc) => {
...
export const parseXMLString = function (pStr, pGanttVar) {
  ...
  if (typeof (<any>window).DOMParser != 'undefined') {
    ...
    typeof (<any>window).ActiveXObject != 'undefined' &&
...
export const getXMLProject = function () {
  let vProject =
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><project xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">';
...
export const getXMLTask = function (pID, pIdx) {
  ...
  if (vIdx >= 0 && vIdx < this.vTaskList.length) {
    ...
    vTask +=
      '<pStart>' +
      formatDateStr(this.vTaskList[vIdx].getStart(), vOutFrmt, this.vLangs[this.vLang]) +
      '</pStart>';
    vTask +=
      '<pEnd>' +
      formatDateStr(this.vTaskList[vIdx].getEnd(), vOutFrmt, this.vLangs[this.vLang]) +
      '</pEnd>';
    vTask +=
      '<pPlanStart>' +
      formatDateStr(this.vTaskList[vIdx].getPlanStart(), vOutFrmt, this.vLangs[this.vLang]) +
      '</pPlanStart>';
    vTask +=
      '<pPlanEnd>' +
      formatDateStr(this.vTaskList[vIdx].getPlanEnd(), vOutFrmt, this.vLangs[this.vLang]) +
      '</pPlanEnd>';
    ...
    let vTmpDiv = newNode(
      ...
      null,
      null,
      this.vTaskList[vIdx].getNotes().innerHTML,
      null,
      null,
      null,
      null,
